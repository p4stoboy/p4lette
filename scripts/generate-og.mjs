...
const TERMINAL = { bg: "#0A0A0A", ink: "#E8E5DD", accent: "#00FF6A" };
...
const fontColorFor = (hex, dark, light) => (luminance(hex) > 0.55 ? dark : light);
...
function buildOgSvg() {
  ...
  // Drop symbol glyphs from nav buttons in the OG: their fallback fonts drag
  // the rest of the text run out of Space Grotesk. The labels alone read fine.
  const buttons = [
    { label: "ADD", bold: true },
    { label: "SHUFFLE" },
    { label: "SAVE" },
  ...
  const navParts = buttons.map((b) => {
    ...
    const x = navX;
    navX += approxW;
    return { text: b.label, x, approxW, bold: b.bold };
  });
  ...
  const swatchH = HALF - swatchTop - 8;
  ...
  const posterCols = PALETTE.map((c, i) => {
    ...
    return `
      ...
      <text x="${x + colW / 2}" y="${swatchTop + swatchH / 2 - 4}" fill="${fg}"
        font-family="Anton" font-size="56" text-anchor="middle"
        letter-spacing="1">${escape(c.name)}</text>
      <text x="${x + colW / 2}" y="${swatchTop + swatchH / 2 + 36}" fill="${fg}"
        font-family="JetBrains Mono" font-size="20" text-anchor="middle"
        opacity="0.8">${escape(c.hex)}</text>`;
  }).join("");
  ...
  const TAPE_H = 36;
  const INDEX_H = 32;
  const termTapeY = TERM_Y;
  const termIndexY = TERM_Y + TAPE_H;
  const termBodyY = TERM_Y + TAPE_H + INDEX_H;
  const termBodyH = H - termBodyY;

  const tapeText =
    "p4lette $ shuffle | save | export | harmony | vault [12]                         --more--";

  const indexCells = PALETTE.map((c, i) => {
    const x = i * colW;
    const num = String(i + 1).padStart(2, "0");
    const lockTag = i === 0 ? "[LOCK]" : `[${(luminance(c.hex) * 100).toFixed(2)}L]`;
    return `
      <line x1="${x}" y1="${termIndexY}" x2="${x}" y2="${termIndexY + INDEX_H}" stroke="${TERMINAL.ink}" stroke-width="1" opacity="0.5" />
      <text x="${x + 10}" y="${termIndexY + 21}" fill="${TERMINAL.accent}"
        font-family="JetBrains Mono" font-size="16">[${num}]</text>
      <text x="${x + colW - 10}" y="${termIndexY + 21}" fill="${TERMINAL.ink}"
        font-family="JetBrains Mono" font-size="14" text-anchor="end" opacity="0.6">${escape(lockTag)}</text>`;
  }).join("");

  const termCols = PALETTE.map((c, i) => {
    const x = i * colW;
    const inner = 18;
    const padTop = 24;
    const swatchY = termBodyY + padTop;
    const swatchHeight = termBodyH - padTop - 90;
    const fg = fontColorFor(c.hex, TERMINAL.bgDark || "#0A0A0A", TERMINAL.ink);
    return `
      <rect x="${x + inner}" y="${swatchY}" width="${colW - inner * 2}" height="${swatchHeight}" fill="${c.hex}" stroke="${TERMINAL.ink}" stroke-width="1" />
      <text x="${x + colW / 2}" y="${swatchY + swatchHeight / 2 + 8}" fill="${fg}"
        font-family="JetBrains Mono" font-size="28" text-anchor="middle"
        letter-spacing="1.5">${escape(c.hex.toUpperCase())}</text>
      <text x="${x + inner + 4}" y="${swatchY + swatchHeight + 28}" fill="${TERMINAL.ink}"
        font-family="JetBrains Mono" font-size="14" opacity="0.85">${escape(c.name.toLowerCase())}</text>
      <text x="${x + colW - inner - 4}" y="${swatchY + swatchHeight + 28}" fill="${TERMINAL.accent}"
        font-family="JetBrains Mono" font-size="14" text-anchor="end">◼</text>`;
  }).join("");

  return `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  <!-- POSTER half -->
  <rect width="${W}" height="${HALF}" fill="${POSTER.bg}" />
  ...
  <text x="${wordmarkX}" y="${tagLineY}" fill="${POSTER.ink}"
    font-family="Space Grotesk" font-weight="700" font-size="20" letter-spacing="3">
    DRAG · LOCK · SHUFFLE · SHARE</text>
  ${posterCols}

  <!-- 3px ink rule between halves -->
  <line x1="0" y1="${HALF}" x2="${W}" y2="${HALF}" stroke="${POSTER.ink}" stroke-width="6" />

  <!-- TERMINAL half -->
  <rect x="0" y="${TERM_Y}" width="${W}" height="${HALF}" fill="${TERMINAL.bg}" />
  <text x="20" y="${termTapeY + 24}" fill="${TERMINAL.accent}"
    font-family="JetBrains Mono" font-size="16">${escape(tapeText)}</text>
  <line x1="0" y1="${termIndexY}" x2="${W}" y2="${termIndexY}" stroke="${TERMINAL.ink}" stroke-width="1" opacity="0.4" stroke-dasharray="4 4" />
  ${indexCells}
  <line x1="0" y1="${termBodyY}" x2="${W}" y2="${termBodyY}" stroke="${TERMINAL.ink}" stroke-width="1" opacity="0.4" stroke-dasharray="4 4" />
  ${termCols}
  <text x="${W - 24}" y="${H - 16}" fill="${TERMINAL.ink}"
    font-family="JetBrains Mono" font-size="13" text-anchor="end" opacity="0.5">p4lette.app</text>
</svg>`;
}
...
function rasterise(svg, width) {
  return new Resvg(svg, {
    font: {
      ...
      // Grotesk/JetBrains Mono (★ ＋ ♥ ⚄ ☾ ◼) resolve via Noto Sans Symbols /
...
async function main() {
  ...
  await writeFile(join(PUBLIC, "apple-touch-icon.png"), rasterise(appleSvg, 180));
