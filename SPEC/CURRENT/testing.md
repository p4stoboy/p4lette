...
| `src/functions/generate_palette.test.ts`        | `it.each([1,2,5,8,12])` → that many valid `#rrggbb` strings; `[]` for non-positive count; deterministic given a fixed `rnd`; "coherent set" — OKLCH lightness span > 0.3 across 8 pseudo-random seeds; two consecutive `Math.random()`-driven calls differ. (imports `oklch` from `culori`)                                                                                                                                                                                                                                                                                                                                                                        |
...
| `src/functions/harmony.test.ts`                 | `harmony` + `harmonyRyb` — `it.each` over all 7 `HarmonyKind`s → exact count + valid hex. `harmony`: complementary ≈ 180° apart in OKLCH hue; triadic all pairs > 100°; tetradic all pairs > 40°; monochrome holds hue/chroma with monotonic lightness; shades monotonic lightness. `harmonyRyb`: complementary of red → green family (Itten); triadic of red → yellow-ish + blue-ish; rotation harmonies stay at the seed's lightness (Δ < 0.05) and carry ≥ 0.7× the seed's chroma (no washout). (imports `oklch` from `culori`)                                                                                                                                 |
| `src/functions/resolve_export_template.test.ts` | Single-prop bare value; full object as JSON (`name`/`hex`/`rgb`/`hsl`/`hsv`/`oklch` keys); `$[1,2].name$` array; `$[all].hex$`; repeated tokens deterministic; `[ERROR: no color 3]` / `[ERROR: no prop cmyk]`; missing ids filtered from arrays; hsv/oklch token props.                                                                                                                                                                                                                                                                                                                                                                                           |
...
| `src/functions/tones.test.ts`                   | `TONE_METHODS` ids in order `[ditto, oklch, hsv]`, non-blank `label`/`caption`; `it.each` per method → 11 valid hexes; descending OKLCH lightness (lightest first); lightness span > 0.6; includes a swatch within 0.15 L of the seed. (imports `oklch` from `culori`)                                                                                                                                                                                                                                                                                                                                                                                             |
...
```json
{
  ...
  "setup": "src/setupTests.ts → import '@testing-library/jest-dom/vitest'",
  ...
  "commands": {
    ...
    "lint": "eslint . — exits non-zero on the one pre-existing PosterEditTray.tsx:31 react-hooks/set-state-in-effect error",
  ...
  "uncovered": [
    ...
    "src/functions/tones_tailwind_data.ts",
    "src/App.tsx",
    "src/index.tsx",
...
flowchart LR
  ...
  lc["npm run lint"] --> es["eslint . → 1 known error: PosterEditTray.tsx:31"]
