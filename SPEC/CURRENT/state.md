...
- **`Provider({ children, initialState? })`**:
  ...
  - Action callbacks — each a `useCallback([])` dispatching the matching action: `addColor, deleteColor, updateColor, reorderColor, toggleLock, randomizeUnlocked, replaceAll, setExportTemplate, setExportVisible, setNameList, setColorMode, setEditSpace, setGenConfig`.
