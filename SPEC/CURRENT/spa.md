...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop) + `×`. Two sections, **side by side on desktop (50/50, each scrolls independently); stacked on mobile (the tray scrolls through both)**:
  ...
  - **TONES** — `SEED COLOR` picker (same shape, no wheel toggle) + the `TONE_METHODS` rows — each `scale = tones(base, m.id)` → 11-swatch strip + `m.label` + caption (`m.caption`; for the `ditto` row, suffixed `· matched {dittoMatch(base).shade} ({.method})` — e.g. `· matched amber-700 (blend)`) + `USE` (`onApply(scale)`). Holds local `base`; `match = dittoMatch(base)` computed once per render.
  - Imports `harmony, harmonyRyb, HarmonyKind` from `../../functions/harmony`, `TONE_METHODS, tones, dittoMatch` from `../../functions/tones`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
