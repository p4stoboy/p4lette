...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop: `HARMONY · TONES · FIXERS · PIGMENT · MIX · EFFECTS — …`) + `×`. Body is a **responsive section grid on desktop** (`grid-template-columns: repeat(auto-fit, minmax(340px, 1fr))`, `align-content:start`, the whole body scrolls — sections across when wide, wrapping below when narrow) / a **stacked scrolling column on mobile**. Each section: `borderRight` on desktop, `borderTop` on mobile (non-first). Six sections:
  ...
  - `Toggle` (the tab button) is `nowrap`/`ellipsis` so the longer style/cube/profile/harmony/mode labels don't break a row on narrow viewports.
  - Imports `harmony, harmonyRyb, harmonyHsv, HarmonyKind, PaletteStyle, HARMONY_STYLES, RYB_CUBES, HARMONY_HSV_KINDS` from `../../functions/harmony`, `TONE_METHODS, tones, dittoMatch` from `../../functions/tones`, `CvdType, simulateCvd, snapToGamut, EFFECTS, applyEffect, BLEND_MODES, BlendMode, blendWith` from `../../functions/color_filters`, `PIGMENT_CUBES, pigmentFilter, pigmentWheel, cubeCorners` from `../../functions/pigment`, `MIX_SPACES, MIX_STEPS, MIX_CURVES, MixSpace, MixCurve, mixSteps` from `../../functions/color_mix`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
