...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop: `HARMONY · TONES · FIXERS · PIGMENT — …`) + `×`. Body is a **responsive section grid on desktop** (`grid-template-columns: repeat(auto-fit, minmax(340px, 1fr))`, `align-content:start`, the whole body scrolls — sections across when wide, wrapping below when narrow) / a **stacked scrolling column on mobile**. Each section: `borderRight` on desktop, `borderTop` on mobile (non-first). Four sections:
  ...
  - `Toggle` (the tab button) is `nowrap`/`ellipsis` so the longer style/cube/profile/harmony labels don't break a row on narrow viewports.
  - Imports `harmony, harmonyRyb, harmonyHsv, HarmonyKind, PaletteStyle, HARMONY_STYLES, RYB_CUBES, HARMONY_HSV_KINDS` from `../../functions/harmony`, `TONE_METHODS, tones, dittoMatch` from `../../functions/tones`, `CvdType, simulateCvd, snapToGamut` from `../../functions/color_filters`, `PIGMENT_CUBES, pigmentFilter, pigmentWheel, cubeCorners` from `../../functions/pigment`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
...
```json
{
  ...
  "components": {
    ...
    "PosterToolsTray": "full-surface overlay (fixed inset:0, zIndex 55); four sections in a responsive grid (auto-fit minmax(340px,1fr), body scrolls) on desktop / stacked column on mobile — HARMONY (base picker + OKLCH|RYB|HSV space toggle + a styles/cubes/none variant picker + harmony rows: oklch/ryb → 7 HARMONIES via harmony(base,kind,style)|harmonyRyb(base,kind,cube) [style now affects mono/shades via tintsShades]; hsv → HARMONY_HSV_KINDS via harmonyHsv(base,kind) incl. pentadic/hexadic/compound/dbl-comp), TONES (seed picker + 4 TONE_METHODS rows ditto/oklch/hsv/gen; ditto row caption suffixed with dittoMatch(base) → 'matched <ramp>-<step> (<method>)'), FIXERS (no picker; CVD prot/deuter/trit previews of the live palette + an IN-GAMUT sRGB snap via simulateCvd/snapToGamut), PIGMENT (no picker; a PROFILE picker over PIGMENT_CUBES + FILTER=pigmentFilter(palette,cube) — the print-like re-render — / WHEEL=pigmentWheel(cube) / THIS CUBE=cubeCorners(cube)); each USE→onApply the transformed palette; Toggle is nowrap/ellipsis; replaced PosterHarmonyDrawer + PosterTonesDrawer",
