...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop: `HARMONY · TONES · FIXERS · GENERATE — …`) + `×`. Body is a **responsive section grid on desktop** (`grid-template-columns: repeat(auto-fit, minmax(340px, 1fr))`, `align-content:start`, the whole body scrolls — sections across when wide, wrapping below when narrow) / a **stacked scrolling column on mobile**. Each section: `borderRight` on desktop, `borderTop` on mobile (non-first). Four sections:
  - **HARMONY** — `BASE COLOR` picker (swatch + hex `<input>` + a row of buttons to pick `base` from the current palette) + a 2-row control: row 1 is the `OKLCH` / `RYB` space toggle; row 2 is a variant picker — when `oklch`, the `HARMONY_STYLES` (`DEFAULT/SQUARE/TRIANGLE/CIRCLE/DIAMOND`); when `ryb`, the `RYB_CUBES` pigment wheels (`ITTEN/GOETHE/BEZOLD/MUNSELL/CHEVREUL`). Then rows `ANALOGOUS, COMPLEMENTARY, TRIADIC, TETRADIC, SPLIT-COMP, MONOCHROME, SHADES` — each `colors = space==="ryb" ? harmonyRyb(base,kind,cube) : harmony(base,kind,style)` → swatch strip + label + `USE` (`SmallBtn` → `onApply(colors)`). Holds local `base`, `space`, `style`, `cube`. (`monochrome`/`shades` ignore style/cube — always the local lightness ramp.)
  - **TONES** — `SEED COLOR` picker (same shape, no controls) + the `TONE_METHODS` rows — each `scale = tones(base, m.id)` → 11-swatch strip + `m.label` + caption (`m.caption`; for the `ditto` row, suffixed `· matched {dittoMatch(base).shade} ({.method})` — e.g. `· matched amber-700 (blend)`) + `USE` (`onApply(scale)`). Holds local `base`; `match = dittoMatch(base)` computed once per render.
  ...
  - `Toggle` (the tab button) is `nowrap`/`ellipsis` so the longer style/cube/strategy labels don't break a row on narrow viewports. `RangeRow` = a labelled `<input type="range">` (`accentColor: ink`) with `min/max/step` from a meta object.
  - Imports `usePalette` from `../../context/PaletteContext`, `harmony, harmonyRyb, HarmonyKind, PaletteStyle, HARMONY_STYLES, RYB_CUBES` from `../../functions/harmony`, `TONE_METHODS, tones, dittoMatch` from `../../functions/tones`, `CvdType, simulateCvd, snapToGamut` from `../../functions/color_filters`, `GEN_STRATEGIES, GenStrategy, RAMP_PARAM_META, RampParams, defaultRampParams, generatePalette` from `../../functions/generate_palette`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
...
```json
{
  ...
  "components": {
    ...
    "PosterToolsTray": "full-surface overlay (fixed inset:0, zIndex 55); four sections in a responsive grid (auto-fit minmax(340px,1fr), body scrolls) on desktop / stacked column on mobile — HARMONY (base picker + OKLCH|RYB space toggle + a styles/cubes variant picker + 7 harmony rows; harmony(base,kind,style) | harmonyRyb(base,kind,cube); mono/shades ignore style/cube), TONES (seed picker + TONE_METHODS rows; ditto row caption suffixed with dittoMatch(base) → 'matched <ramp>-<step> (<method>)'), FIXERS (no picker; CVD prot/deuter/trit previews of the live palette + an IN-GAMUT sRGB snap via simulateCvd/snapToGamut), GENERATE (reads genStrategy/genParams/setGenConfig from usePalette(); STRATEGY toggle GEN_STRATEGIES + rampensau RangeRow sliders from RAMP_PARAM_META + a PREVIEW row of generatePalette(...) with REGEN + USE → onApply(preview) AND setGenConfig so the nav SHUFFLE uses it); each USE→onApply; Toggle is nowrap/ellipsis; replaced PosterHarmonyDrawer + PosterTonesDrawer",
