...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop: `HARMONY · TONES · FIXERS — …`) + `×`. Body is a **responsive section grid on desktop** (`grid-template-columns: repeat(auto-fit, minmax(340px, 1fr))`, `align-content:start`, the whole body scrolls — 3 sections across when wide, wrapping below when narrow) / a **stacked scrolling column on mobile**. Each section: `borderRight` on desktop, `borderTop` on mobile (non-first). Three sections:
  ...
  - `Toggle` (the tab button) is `nowrap`/`ellipsis` so the longer style/cube labels don't break a row on narrow viewports.
  - Imports `harmony, harmonyRyb, HarmonyKind, PaletteStyle, HARMONY_STYLES, RYB_CUBES` from `../../functions/harmony`, `TONE_METHODS, tones, dittoMatch` from `../../functions/tones`, `CvdType, simulateCvd, snapToGamut` from `../../functions/color_filters`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
...
```json
{
  ...
  "components": {
    ...
    "PosterToolsTray": "full-surface overlay (fixed inset:0, zIndex 55); three sections in a responsive grid (auto-fit minmax(340px,1fr), body scrolls) on desktop / stacked column on mobile — HARMONY (base picker + OKLCH|RYB space toggle + a styles/cubes variant picker + 7 harmony rows; harmony(base,kind,style) | harmonyRyb(base,kind,cube); mono/shades ignore style/cube), TONES (seed picker + TONE_METHODS rows; ditto row caption suffixed with dittoMatch(base) → 'matched <ramp>-<step> (<method>)'), FIXERS (no picker; CVD prot/deuter/trit previews of the live palette + an IN-GAMUT sRGB snap via simulateCvd/snapToGamut, USE applies the transformed palette); Toggle is nowrap/ellipsis; USE→onApply; replaced PosterHarmonyDrawer + PosterTonesDrawer",
