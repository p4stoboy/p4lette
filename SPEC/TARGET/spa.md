...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop: `HARMONY · TONES · FIXERS · PIGMENT · MIX · EFFECTS · GENERATE — …`) + `×`. The body renders one section per entry in the **`TRAY_SECTIONS` registry** (`src/skins/poster/tools/index.ts` — `{ key, label, Body }[]`) — a **responsive section grid on desktop** (`grid-template-columns: repeat(auto-fit, minmax(340px, 1fr))`, `align-content:start`, the whole body scrolls — sections across when wide, wrapping below when narrow) / a **stacked scrolling column on mobile**; each section = `<div sectionStyle><div subHeaderStyle>{label}</div><Body ink isMobile palette onApply/></div>` (`borderRight` on desktop, `borderTop` on mobile non-first). The sections (each `Body` a file under `src/skins/poster/tools/`; shared building blocks `BasePicker`/`SwatchRow`/`Toggle`/`RangeRow` in `tools/shared.tsx`, the layout-style helpers `sectionStyle`/`subHeaderStyle`/`rowsStyle` + `BodyProps` in `tools/styles.ts`):
...
```json
{
  ...
  "components": {
    ...
    "PosterToolsTray": "full-surface overlay (fixed inset:0, zIndex 55); body = one section per TRAY_SECTIONS entry (src/skins/poster/tools/index.ts — {key,label,Body}) in a responsive grid (auto-fit minmax(340px,1fr), body scrolls) on desktop / stacked column on mobile — HARMONY (base picker + OKLCH|RYB|HSV space toggle + a styles/cubes/none variant picker + harmony rows: oklch/ryb → 7 HARMONIES via harmony(base,kind,style)|harmonyRyb(base,kind,cube) [style now affects mono/shades via tintsShades]; hsv → HARMONY_HSV_KINDS via harmonyHsv(base,kind) incl. pentadic/hexadic/compound/dbl-comp), TONES (seed picker + 4 TONE_METHODS rows ditto/oklch/hsv/gen; ditto row caption suffixed with dittoMatch(base) → 'matched <ramp>-<step> (<method>)'), FIXERS (no picker; CVD prot/deuter/trit previews of the live palette + an IN-GAMUT sRGB snap via simulateCvd/snapToGamut), PIGMENT (no picker; a PROFILE picker over PIGMENT_CUBES + FILTER=pigmentFilter(palette,cube) — the print-like re-render — / WHEEL=pigmentWheel(cube) / THIS CUBE=cubeCorners(cube)), MIX (two FROM/TO pickers + steps/space/curve toggles + mixSteps(from,to,n,space,curve)), EFFECTS (no seed; BLEND OVER picker + BLEND_MODES toggle + 8 EFFECTS preset rows via applyEffect + a BLEND row via blendWith), GENERATE (reads genStrategy/genParams/setGenConfig from usePalette(); STRATEGY toggle GEN_STRATEGIES + rampensau RangeRow sliders from RAMP_PARAM_META + a PREVIEW row of generatePalette(...) with REGEN + USE → onApply(preview) AND setGenConfig so the nav SHUFFLE uses it); each USE→onApply the transformed palette; Toggle is nowrap/ellipsis; each Body a file under tools/, BasePicker/SwatchRow/Toggle/RangeRow in tools/shared.tsx, sectionStyle/subHeaderStyle/rowsStyle/BodyProps in tools/styles.ts",
