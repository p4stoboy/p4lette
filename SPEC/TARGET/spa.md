...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop) + `×`. Two sections, **side by side on desktop (50/50, each scrolls independently); stacked on mobile (the tray scrolls through both)**:
  ...
  - Imports `harmony, harmonyRyb, HarmonyKind, PaletteStyle, HARMONY_STYLES, RYB_CUBES` from `../../functions/harmony`, `TONE_METHODS, tones` from `../../functions/tones`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
...
```json
{
  ...
  "components": {
    ...
    "PosterToolsTray": "full-surface overlay (fixed inset:0, zIndex 55); HARMONY section (base picker + OKLCH|RYB space toggle + a styles/cubes variant picker + 7 harmony rows; harmony(base,kind,style) | harmonyRyb(base,kind,cube)) and TONES section (seed picker + TONE_METHODS rows) — side-by-side on desktop / stacked on mobile; USE→onApply; replaced PosterHarmonyDrawer + PosterTonesDrawer",
