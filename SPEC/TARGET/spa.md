...
- `PosterToolsTray.tsx` — `position:fixed inset:0` full-surface overlay (`zIndex:55`, fade-in `toolsIn`), header `TOOLS` (+ a hint line on desktop: `HARMONY · TONES · FIXERS · PIGMENT · MIX · EFFECTS · GENERATE — …`) + `×`. Body is a **responsive section grid on desktop** (`grid-template-columns: repeat(auto-fit, minmax(340px, 1fr))`, `align-content:start`, the whole body scrolls — sections across when wide, wrapping below when narrow) / a **stacked scrolling column on mobile**. Each section: `borderRight` on desktop, `borderTop` on mobile (non-first). Seven sections:
  - **HARMONY** — `BASE COLOR` picker (swatch + hex `<input>` + a row of buttons to pick `base` from the current palette) + a 2-row control: row 1 is the `OKLCH` / `RYB` / `HSV` space toggle; row 2 is a variant picker — when `oklch`, the `HARMONY_STYLES` (`DEFAULT/SQUARE/TRIANGLE/CIRCLE/DIAMOND`); when `ryb`, the `RYB_CUBES` pigment wheels (`ITTEN/GOETHE/BEZOLD/MUNSELL/CHEVREUL`); when `hsv`, none (the HSV harmonies have no variant). Then the rows — for `oklch`/`ryb` the 7 `HARMONIES` (`ANALOGOUS, COMPLEMENTARY, TRIADIC, TETRADIC, SPLIT-COMP, MONOCHROME, SHADES`) → `colors = space==="ryb" ? harmonyRyb(base,kind,cube) : harmony(base,kind,style)`; for `hsv` the `HARMONY_HSV_KINDS` (`COMPLEMENTARY ANALOGOUS TRIADIC TETRADIC SPLIT-COMP PENTADIC HEXADIC COMPOUND DBL-COMP`) → `colors = harmonyHsv(base, kindKey)` — each row a swatch strip + label + `USE` (`SmallBtn` → `onApply(colors)`). Holds local `base`, `space`, `style`, `cube`. (In OKLCH the `style` toggle now affects MONOCHROME/SHADES too — via pro-color-harmonies `tintsShades`.)
  ...
  - **EFFECTS** — no seed; reads the **live palette**. A `BLEND OVER` picker (same shape) whose `children` are a 2-row `BLEND_MODES` picker (`MULTIPLY SCREEN OVERLAY SOFT-LIGHT` / `HARD-LIGHT DARKEN LIGHTEN DIFFERENCE`). Rows: one `SwatchRow` per `EFFECTS` entry (`GRAYSCALE SEPIA INVERT SATURATE+ DESATURATE CONTRAST+ HUE +30 HUE -30` → `applyEffect(hexes, key)`), then `BLEND · {mode}` → `blendWith(hexes, over, mode)` (caption `palette × {over}`). Each `USE`→`onApply`. Holds local `over`, `mode`.
  - **GENERATE** — reads `genStrategy`/`genParams`/`setGenConfig` from `usePalette()` (the only body that does). A `STRATEGY` picker (`GEN_STRATEGIES` `Toggle` row — `RAMPENSAU SWEEP / POLINE ANCHORS / PLAIN RANDOM`); when `rampensau`, six `RangeRow` sliders (`SAT LO/HI`, `LIGHT LO/HI`, `HUE SPAN`, `CURVE ACCENT`) configured from `RAMP_PARAM_META`. A `PREVIEW` `SwatchRow` of `generatePalette(palette.length, strategy, Math.random, params)` + a `REGEN` `SmallBtn` (re-rolls the preview) + `USE` — which `onApply(preview)` **and** `setGenConfig({strategy, params: rampensau ? params : null})` so the nav SHUFFLE button (and the `Space` shortcut) use the chosen strategy/params afterward. Holds local `strategy`, `params`, `preview` (re-rolled on strategy switch / a slider move / REGEN — never in an effect).
  ...
  - Imports `usePalette` from `../../context/PaletteContext`, `harmony, harmonyRyb, harmonyHsv, HarmonyKind, PaletteStyle, HARMONY_STYLES, RYB_CUBES, HARMONY_HSV_KINDS` from `../../functions/harmony`, `TONE_METHODS, tones, dittoMatch` from `../../functions/tones`, `CvdType, simulateCvd, snapToGamut, EFFECTS, applyEffect, BLEND_MODES, BlendMode, blendWith` from `../../functions/color_filters`, `PIGMENT_CUBES, pigmentFilter, pigmentWheel, cubeCorners` from `../../functions/pigment`, `MIX_SPACES, MIX_STEPS, MIX_CURVES, MixSpace, MixCurve, mixSteps` from `../../functions/color_mix`, `GEN_STRATEGIES, GenStrategy, RAMP_PARAM_META, RampParams, defaultRampParams, generatePalette` from `../../functions/generate_palette`, `SmallBtn` from `./Backdrop`. (Replaced the former `PosterHarmonyDrawer` + `PosterTonesDrawer`.)
...
```json
{
  ...
  "components": {
    ...
    "PosterToolsTray": "full-surface overlay (fixed inset:0, zIndex 55); seven sections in a responsive grid (auto-fit minmax(340px,1fr), body scrolls) on desktop / stacked column on mobile — HARMONY (base picker + OKLCH|RYB|HSV space toggle + a styles/cubes/none variant picker + harmony rows: oklch/ryb → 7 HARMONIES via harmony(base,kind,style)|harmonyRyb(base,kind,cube) [style now affects mono/shades via tintsShades]; hsv → HARMONY_HSV_KINDS via harmonyHsv(base,kind) incl. pentadic/hexadic/compound/dbl-comp), TONES (seed picker + 4 TONE_METHODS rows ditto/oklch/hsv/gen; ditto row caption suffixed with dittoMatch(base) → 'matched <ramp>-<step> (<method>)'), FIXERS (no picker; CVD prot/deuter/trit previews of the live palette + an IN-GAMUT sRGB snap via simulateCvd/snapToGamut), PIGMENT (no picker; a PROFILE picker over PIGMENT_CUBES + FILTER=pigmentFilter(palette,cube) — the print-like re-render — / WHEEL=pigmentWheel(cube) / THIS CUBE=cubeCorners(cube)), MIX (two FROM/TO pickers + steps/space/curve toggles + mixSteps(from,to,n,space,curve)), EFFECTS (no seed; BLEND OVER picker + BLEND_MODES toggle + 8 EFFECTS preset rows via applyEffect + a BLEND row via blendWith), GENERATE (reads genStrategy/genParams/setGenConfig from usePalette(); STRATEGY toggle GEN_STRATEGIES + rampensau RangeRow sliders from RAMP_PARAM_META + a PREVIEW row of generatePalette(...) with REGEN + USE → onApply(preview) AND setGenConfig so the nav SHUFFLE uses it); each USE→onApply the transformed palette; Toggle is nowrap/ellipsis; replaced PosterHarmonyDrawer + PosterTonesDrawer",
