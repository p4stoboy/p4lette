...
- `PosterSavedDrawer.tsx` — **desktop**: a flex child of `PosterSkin`'s content-row side-panel slot (`width:100% height:100%`, sized + slid in by the slot — the `borderLeft` is on the slot); **mobile**: a `<Backdrop align="bottom">` bottom sheet (`maxHeight:88vh`, `borderTop`). Header `SAVE / LOAD` + `×`; sub-bar `{list.length} SAVED · LOCAL` + `♥ SAVE PALETTE` (`onSave`). Body: empty → big `NOTHING / HERE / YET.`; else per `s`: swatch strip + `s.name` + `new Date(s.createdAt).toLocaleDateString() · {n} colors` + `LOAD` (`onLoad(s.hexes)`) / `DEL` (`onDelete(s.id)`).
...
```json
{
  ...
  "posterSkin": {
    ...
    "shortcuts": {
      ...
      "e": "toggle showExport (closes showTools/showSaved)",
      "h": "toggle showTools (closes showExport/showSaved)",
      "?": "toggle showAbout",
  ...
  "components": {
    ...
    "PosterSavedDrawer": "desktop: flex child of the side-panel slot (width/height 100%); mobile: Backdrop-backed bottom sheet (88vh); ♥ SAVE PALETTE; entries name+date+swatches → LOAD/DEL",
